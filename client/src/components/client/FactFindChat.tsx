...
export default function FactFindChat({ onComplete }: FactFindChatProps) {
  ...
  const { sessionId, createSession, saveAnswer, getSessionAnswers } = useFactFind();
  ...
  useEffect(() => {
    if (questions.length > 0) {
      setProgress(Math.min(100, (currentQuestionIndex / questions.length) * 100));
    }
    ...
    setMessages([
      {
        id: "welcome",
        role: "assistant",
        content: "Hello! I'm your AI assistant and I'll be helping you complete your insurance fact find form. Let's start with some basic information.",
      }
    ]);
              ...
              <div className="mt-2">
                ...
                <div className="mt-2 flex justify-end">
                  <Button 
                    ...
                    onClick={() => message.questionId && handleSpecialInput(message.questionId, currentInput)}
                  >
                    Submit
                  </Button>
                </div>
                ...
                {message.options.map((option, index) => (
                  <div key={index} className="flex items-center">
                    <input 
                      ...
                      onChange={() => message.questionId && handleSpecialInput(message.questionId, option)}
                    />
                    <label htmlFor={`option${index}`} className="ml-2 block text-sm text-gray-900">
                      {option}
                    </label>
                  </div>
                ))}
              ...
              <div className="flex items-center justify-center space-x-4">
                <div className="flex items-center">
                  <input 
                    ...
                    name={`question-${message.questionId}`}
                    className="h-4 w-4 text-primary border-gray-300 focus:ring-primary" 
                    onChange={() => message.questionId && handleSpecialInput(message.questionId, "Yes")}
                  />
                  <label htmlFor="yes" className="ml-2 block text-sm text-gray-900">Yes</label>
                </div>
                <div className="flex items-center">
                  <input 
                    id="no" 
                    ...
                    name={`question-${message.questionId}`}
                    className="h-4 w-4 text-primary border-gray-300 focus:ring-primary" 
                    onChange={() => message.questionId && handleSpecialInput(message.questionId, "No")}
                  />
                  <label htmlFor="no" className="ml-2 block text-sm text-gray-900">No</label>
                </div>
              </div>
            )}
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-sm overflow-hidden">
      ...
      <div className="border-b p-4">
        ...
        <p className="text-sm text-gray-500">Please answer the following questions to help us understand your insurance needs.</p>
      </div>
